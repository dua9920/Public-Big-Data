...
-- cslee dafault로 설정해서 cslee. 구문 안써줘도 됨.
-- DBeaver 에서 [cslee] 스키마 선택 - 오른쪽 마우스 Set as Default
...
-- 해당 컬럼만 지정하기
select cust_name, reg_num from tb_cust;
...
from tb_emp;
-- 2) 결과 일부만 발췌
-- 원 데이터로는 29명이 나와야하지만 마지막에 데이터 하나 임의로 추가해서 30명 맞게 나오는 것 확인.
사원이름|소속 |직무  |연봉       
----+---+----+---------+
...
-- 3)
select count(*) from tb_emp where salary >= 50000000;
select emp_name 사원이름, org_cd 소속, position 직무, salary 연봉
from tb_emp
...
-- 예제6: where 조건절 - like
-- 1) 김씨 성을 가진 직원들의 정보를 조회하는 where절 작성
...
김미정 |   |    |2014-01-02 00:00:00|

-- 2) 이름의 두번째가 '승'인 임직원의 정보를 조회
select emp_name 사원이름, org_cd 팀코드, position 직책, ent_date 입사일자
from tb_emp
...
-- 예제8: where 조건 - 부정 연산자
-- 1) 영업3팀(10)의 사원중에서 '팀장'을 제외한 나머지 직원들의 자료를 찾는다.
